...
chaptersButton.addEventListener('click', async () => {
    ...
    if (currentLine < tableOfContents.length) {
        const prompt = `Based on the ${genre} chapter title: "${tableOfContents[currentLine]}" with keywords: ${keywords.join(', ')}, generate the comprehensive explanation, and if necessary add this: example, instruction, tips, or code in python.`;
        console.log(prompt);
